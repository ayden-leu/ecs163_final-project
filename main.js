...
function initSearchInput(){
	...
	function getMatchingLocations(){
		...
		const matchingLocations = allLocationNames
            .filter((data) => data.name && data.name.toLowerCase().includes(userQuery))
            .slice(0, 5);
		// expected format:
...
function createMapVisual(){
    ...
    const countyLabelLayer = layerGroup.append("g").attr("class", "county-labels");
	...
	function zoomMap(event){
		...
		// change opacity of county labels based on zoom amount
		...
		// apply opacity and also adjust font size
		countyLabelLayer
			.selectAll("text")
		...
		// change opacity of city labels based on zoom amount
		...
		// apply opacity and also adjust font size
		// starts smaller and scales more gently compared to county labels
		cityLabelLayer
			.selectAll("text")
...
function createFireTooltip(){
	d3.select("body").append("div")
		.attr("id", "fire-tooltip")
}

function createFires(year){
	const filteredFires = mapObj.feature.fire.filter(f => f.properties.YEAR_ === year);

	const firePaths = mapObj.layer.fire
		.selectAll("path.fire")
		.data(filteredFires, (data) => 
			data.properties.IRWINID || JSON.stringify(data.geometry)
		);
...
function createLineGraph(){
	...
	const lineGraph = d3.select("#graph")
    ...
    const lineGraphRangeX = d3.scaleTime()
        .range([0, lineGraphObj.width - style.lineGraph.padding.left - style.lineGraph.padding.right])
    ;
    // x axis visual
    ...
    const lineGraphRangeY = d3.scaleLinear()
        .range([lineGraphObj.height - style.lineGraph.padding.top - style.lineGraph.padding.bottom, 0])
        .nice();
    ...
    const sectionHighlighter = lineGraph.append("rect")
        ...
        .attr("height", 
			lineGraphObj.height - style.lineGraph.padding.top - style.lineGraph.padding.bottom
		)
    ;
    ...
    lineGraph.append("clipPath")
        ...
        .append("rect")
            .attr("x", 0)
            .attr("y", 0)
            .attr("width", lineGraphObj.width - style.lineGraph.padding.left - style.lineGraph.padding.right)
            .attr("height", lineGraphObj.height - style.lineGraph.padding.top - style.lineGraph.padding.bottom)
			.attr("fill", "black")
    ;
...
function updateLineGraph(regionData = null){
    ...
    if(regionData !== null){
		...
		lineGraphObj.currentRegionData = (lsad === undefined)?
        ...
        lineGraphObj.y.domain = [  // round down as the precision caused errors in the min max evaluations
            d3.min(lineGraphObj.currentRegionData, entry => Math.floor(entry.value)),
            d3.max(lineGraphObj.currentRegionData, entry => Math.floor(entry.value))
        ];
    }
    ...
    lineGraphObj.detectionArea
        ...
        .on("mousemove", function(event){
			...
			const circleRadius = Number(d3.select("#lineGraphFocusCircle").style("r").replace("px", ""));
...
function updateLineGraphDomainToAll(regionData){
	d3.select("#lineGraphSectionHighlighter").style("opacity", style.lineGraph.highlighter.opacityVisible);
...
function updateLineGraphDomainToYear(year = null){
    ...
    lineGraphObj.y.domain = [
        d3.min(lineGraphObj.currentRegionData, entry => getValueInDateRange(entry, true)),
        d3.max(lineGraphObj.currentRegionData, entry => getValueInDateRange(entry, false))
    ];
...
function zoomToRegion(regionData){
	...
	const x = (bounds[0][0] + bounds[1][0]) / 2;
	const y = (bounds[0][1] + bounds[1][1]) / 2;
	const scale = Math.max(1, Math.min(8, 0.9 / Math.max(dx / mapObj.width, dy / mapObj.height)));

	const sidebarOffset = 300;
	const translate = [(mapObj.width - sidebarOffset) / 2 - scale * x, mapObj.height / 2 - scale * y];
...
function onRegionClicked(regionData){
	...
	const regionName = (lsad === undefined)?
