...
lineGraph
	...
	lineGraphObj.focusCircle = focusCircle;
...
function createLineGraphTooltip() {
	d3.select("body").append("div")
...
// yxaxis
	...
	lineGraphObj.detectionArea
		.on("mouseover", function () {
			lineGraphObj.focusCircle.style("opacity", 1);
		})
		.on("mousemove", function (event) {
			...
			d3.select("#lineGraphTooltip")
				.classed("visible", true)
				.style("left", tooltipX + "px")
				.style("top", tooltipY + "px").html(`
					<strong>${tooltipDate}</strong><br/>
					<strong>$</strong> ${tooltipValue}<br/>
				`);
		})
		.on("mouseout", function () {
			lineGraphObj.focusCircle.style("opacity", 0);
			d3.select("#lineGraphTooltip").classed("visible", false);
		});
}
